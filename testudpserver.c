...
_poll(void * ud) {// socket thread execute
	...
	for (;;) {
		...
		case SOCKET_EXIT:
			return NULL;
		case SOCKET_DATA: //tcp
			printf("message tcp(%lu) [id=%d] size=%d data=%s\n",result.opaque,result.id, result.ud, result.data);
            free(result.data);
			break;
        case SOCKET_UDP:
            printf("message udp(%lu) [id=%d] size=%d data=%s\n", result.opaque, result.id, result.ud, result.data);
            int addrsz = 0;
            const struct socket_udp_address *addr = socket_server_udp_address(ss, &result, &addrsz);
...
//            int ret = socket_server_send(ss, &buf);
            int ret = socket_server_udp_send(ss, addr, &buf);
            if(ret == 0) {
            ...
            free(result.data);
            break;
		case SOCKET_CLOSE: //tcp
			printf("close(%lu) [id=%d]\n",result.opaque,result.id);
			break;
		case SOCKET_OPEN: //tcp
			printf("open(%lu) [id=%d] data=%s\n",result.opaque, result.id, result.data);
			break;
        case SOCKET_ACCEPT: //tcp
...
test(struct socket_server *ss) {
    ...
    int id = socket_server_udp(ss, 200, "0.0.0.0", 1234);
